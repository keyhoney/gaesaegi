...
(async function () {
  ...
  window.firebaseData = {
    ...
    async getQuestionLastCorrectTime(qid) {
      try {
        ...
        if (snap.exists()) {
          ...
          return data?.lastCorrectAt;
        }
        return null;
      } catch (error) {
        console.error('쿨다운 조회 오류:', error);
        return null;
      }
    },
    ...
    // ===== 일일 코인 획득 제한 =====
    async getDailyCoinRewards(dateKey) {
      try {
        const { user, db } = await withUser();
        const { doc, getDoc } = await import('https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js');
        const ref = doc(db, 'users', user.uid, 'dailyCoinRewards', dateKey);
        const snap = await getDoc(ref);
        return snap.exists() ? snap.data() : { count: 0, lastRewardAt: null };
      } catch (_) { return { count: 0, lastRewardAt: null }; }
    },

    async addDailyCoinReward(dateKey) {
      try {
        const { user, db } = await withUser();
        const { doc, setDoc, serverTimestamp } = await import('https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js');
        const ref = doc(db, 'users', user.uid, 'dailyCoinRewards', dateKey);
        const current = await this.getDailyCoinRewards(dateKey);
        await setDoc(ref, { 
